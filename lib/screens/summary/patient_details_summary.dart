...
class PatientDetailsForm extends StatelessWidget {
                ...
                children: [
                  ...
                  if (_hasValue(details, 'previous_history') ||
                    ...
                    if (details['previous_history'] is List &&
                      ...
                      _buildText(
                        (details['previous_history'] as List).join(', '),
                      ),
                  ...
                  if (_hasValue(details, 'TREATMENT/ACTION')) ...[
                    ...
                    if (details['TREATMENT/ACTION'] is List)
                      _buildText(
                        (details['TREATMENT/ACTION'] as List).join(', '),
                      ),
                  ],
                  if (_hasValue(
                    details,
                    ...
                    if (details['HANDLING & IMMOBILISATION ON DEPARTURE']
                        is List)
                      _buildText(
                        (details['HANDLING & IMMOBILISATION ON DEPARTURE']
                                as List)
                            .join(', '),
                      ),
                  ],
                  ...
                  if (_hasValue(details, 'BP (mmHg)') ||
                    ...
                    _buildSectionHeader(
                      context,
                      'On Arrival Vital Signs Record',
                    ),
                    if (_hasValue(details, 'BP (mmHg)'))
                      _buildInfoRow(
                        'BP (mmHg):',
                        details['BP (mmHg)'].toString(),
                      ),
                    if (_hasValue(details, 'RR (min)'))
                      _buildInfoRow(
                        'RR (min):',
                        details['RR (min)'].toString(),
                      ),
                    if (_hasValue(details, 'SPO2 (%)'))
                      _buildInfoRow(
                        'SPO2 (%):',
                        details['SPO2 (%)'].toString(),
                      ),
                  ...
                  if (_hasValue(
                    ...
                    _buildText(
                      details['Other Patient Progress/ Remarks'].toString(),
                    ),
                  ],
                  const SizedBox(height: 20),
      ...
      children: [
        ...
        if (image != null)
          Container(
            ...
            child: Image.memory(image, fit: BoxFit.contain, height: 200),
          )
        else
          Container(
            height: 200,
  ...
  Widget _buildSignOffColumn(
    ...
    return Column(
      children: [
        if (_hasValue(details, nameKey))
          _buildInfoRow(nameLabel, details[nameKey].toString()),
        if (_hasValue(details, icKey))
          _buildInfoRow(icLabel, details[icKey].toString()),
    ...
    return Column(
      children: [
        ...
        if (_hasValue(details, 'endorsed_by_signature'))
          _buildSignatureDisplay(details['endorsed_by_signature']),
      ],
    );
  }
    ...
    return Column(
      children: [
        ...
        if (_hasValue(details, 'received_by_signature'))
          _buildSignatureDisplay(details['received_by_signature']),
      ],
    );
  }
}
